...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress) {
	...
	interruptVectorTable[CLOCKINT_BIT]=interruptVectorInitialAddress+4; // CLOCKINT_BIT=9 Ej 2 V2
}
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case DIV_INST: 
			if (operand2 == 0)
				Processor_RaiseException(DIVISIONBYZERO); //V4 Ej2
			else {
				registerAccumulator_CPU=operand1 / operand2;
				registerPC_CPU++;
			}
		...
		case HALT_INST: 
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)){ // Protected mode
				...
				Processor_RaiseException(INVALIDPROCESSORMODE); //V4 
			}
			break;
		...
		case MEMADD_INST: 
			...
			// Copy the read data to the accumulator register plus the second operand
	...
	//V3 Eje 2-a cambiar el mensaje a 
    //[35]_{06_803_000}_JUMP_-3_0_(PID:_1,_PC:_1,_Accumulator:_-2,_PSW:_0004_[-------------N--]) 
	ComputerSystem_DebugMessage(130, HARDWARE, InstructionNames[operationCode],operand1,operand2,  OperatingSystem_GetExecutingProcessID() ,registerPC_CPU,registerAccumulator_CPU,registerPSW_CPU,Processor_ShowPSW());
...
void Processor_ManageInterrupts() {
	...
	int i;
		for (i=0;i<INTERRUPTTYPES;i++)
			...
			if (Processor_GetInterruptLineStatus(i)&&!Processor_PSW_BitState(INTERRUPT_MASKED_BIT)) {
